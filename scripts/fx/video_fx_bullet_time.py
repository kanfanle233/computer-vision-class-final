...
def open_video_writer(path: str, fps: float, size: tuple[int, int]) -> tuple[cv2.VideoWriter, str]:
    """Prefer AVC output; fall back to mp4v when a platform lacks an H.264 encoder."""
...
def run(args: argparse.Namespace) -> None:
    ...
    writer, output_codec = open_video_writer(args.output, fps, (w, h))
    ...
    print(f"[INFO] output_codec={output_codec}")
