...
def resolve_device(requested):
    """Resolve an accelerator consistently on Windows, macOS, and CPU hosts."""
...
def evaluate(indices, y_true=None, y_pred=None, c_true=None, c_pred=None, tolerance=4., img_scaler=(1, 1), output_bbox=False, output_gt=False):
    ...
    if y_true is not None and y_pred is not None:
        ...
        h_pred = y_pred > float(os.environ.get("TRACKNET_VIS_THRESH", "0.2"))
...
def eval_tracknet(model, data_loader, param_dict):
    ...
    model.eval()
    device = param_dict.get('device') or resolve_device('auto')
    losses = []
...
def eval_inpaintnet(model, data_loader, param_dict):
    ...
    for step, (i, coor_pred, coor, _, _, inpaint_mask) in enumerate(data_prob):
        coor_pred = coor_pred.float().to(device)
        coor = coor.float().to(device)
        inpaint_mask = inpaint_mask.float().to(device)
...
def test_rally(model, rally_dir, param_dict, save_inpaint_mask=False):
    ...
    if inpaintnet is None:
        ...
        if param_dict['eval_mode'] == 'nonoverlap':
            ...
            device = param_dict.get('device', torch.device('cpu'))
            for step, (i, x, y, _, _) in enumerate(data_prob):
                x = x.float().to(device)
                with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default='auto', choices=['auto', 'cpu', 'cuda', 'mps'],
                        help='Device for inference: auto (default), cpu, cuda, or mps')
    ...
    # Resolve device (supports auto, cuda, mps, cpu)
