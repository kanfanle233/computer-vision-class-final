...
整套系统基于 **TrackNet（球检测） + YOLOv8s-pose（球员姿态） + ByteTrack（多目标跟踪） + 透视矫正**，支持 Windows NVIDIA CUDA 与 Apple Silicon MPS 自动加速。跨平台复制运行请先看 [README_PORTABLE.md](README_PORTABLE.md)。
...
这个仓库包含跨平台入口 `run_pipeline.py`，支持 CUDA / MPS / CPU；历史修复记录在 [HANDOVER.md](HANDOVER.md) 里。
...
设备设为 `auto` 时，Windows NVIDIA 环境自动使用 CUDA，Apple Silicon 环境自动使用 MPS。详细安装与 PowerShell 示例见 [README_PORTABLE.md](README_PORTABLE.md)。
