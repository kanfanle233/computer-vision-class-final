...
Built on **TrackNet (shuttle detection) + YOLOv8s-pose (player keypoints) + ByteTrack (ID tracking) + court homography**. The portable launcher `run_pipeline.py` automatically uses NVIDIA CUDA on Windows or MPS on Apple Silicon; see [README_PORTABLE.md](README_PORTABLE.md).
